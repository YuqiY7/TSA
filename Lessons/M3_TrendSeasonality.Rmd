...
#也可以只去除最后几个月，可以正常处理数据。但建议去除整年的，以保证每个月份的数量一致。
...
#data <- data.frame("t"=t,"original"=inflow_data$iHP)

#Fit a linear trend to TS of iHP
linear_trend_model <- lm(inflow_data[,iHP+1]~t) #+1 refers to date column,第一列是日期
...
ts_linear <- ts(linear_trend,start = c(1931,1),frequency = 12)
...
autoplot(ts_inflow_data[,iHP],color="darkblue")+
  autolayer(ts_detrend,series="Detrended",colour = "darkgreen")+
